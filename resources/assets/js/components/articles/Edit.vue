                ...
                <Col span="9" style="padding-left: 10px">
                    ...
                    <Card style="margin-top: 10px">
                        ...
                        <Row>
                            <Col span="11">
                                <FormItem label="原创">
                                    <i-switch size="large" v-model="form.is_original" :true-value="1" :false-value="0">
        ...
        mounted(){
            axios.get('/api/articles/' + this.$route.params.id).then(response=>{
                ...
                for (let topic of topics.values()){
                    ...
                    this.form.topics.push(topic.name)
                }
                this.form.title = response.data.title
                this.form.content = response.data.content
                this.form.description = response.data.description
                this.form.image = response.data.image
                this.form.published_at = response.data.published_at
                this.form.is_original = response.data.is_original
            })
        },
        methods:{
            moreTopics(query){
            ...
            onSubmit(){
                for (let index in this.form.topics){
                    for (let value of this.topics){
                        if (this.form.topics[index] == value.name){
                            this.form.topics[index]=value.id
                        }
                    }
                }
                let data = {
